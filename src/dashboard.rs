...
/// Number of recent rows shown in the inline execution graph of the
/// selected session. Keeps the per-session block small enough to fit
/// alongside the session list (one screenful).
const GRAPH_ROWS_PER_SESSION: usize = 8;
...
impl Dashboard {
    ...
    /// Cheap to call repeatedly: a no-op once the tail is loaded.
    ...
    /// Load the tail for the session at `idx` if it isn't already loaded.
    /// Called by the renderer before drawing the execution graph so the
    ...
    /// Pre-load the tail of every currently-Running session, so the inline
    /// execution graphs are visible on first paint of the Sessions view.
    ...
    /// Read-only handle to the focused session's tail (used by the renderer
    /// when drawing the inline execution graph).
    ...
    /// Poll each loaded tail: re-parse the JSONL from the saved offset,
    /// append new events to the tree, and refresh the session-level status.
    /// Tails that haven't been loaded yet are loaded lazily when their
    /// JSONL has content, so a brand-new session appears within one tick.
    ...
    pub fn move_selection(&mut self, delta: isize) {
        ...
        // Pre-load the tail for the focused session so its inline graph
        // is ready by the time the renderer draws the next frame.
    ...
    if position == dash.selected {
        ...
        if let Some(tail) = dash.tails.get(&s.path) {
            if let Some(err) = &tail.last_error {
                lines.push(Line::from(vec![
                    Span::styled("          warn ", Style::default().fg(Color::Red)),
                    Span::styled(err.clone(), Style::default().fg(Color::Red)),
                ]));
            }
            let recent: Vec<&Node> = tail
                .tree
                .rows
                .iter()
                .rev()
                .take(GRAPH_ROWS_PER_SESSION)
                .collect();
...
fn render_dashboard_row(n: &Node, accent: Color) -> Line<'static> {
    ...
    let (status_glyph, status_color) = match n.status {
        ...
        Status::Done => ("  ok", Color::Green),
        Status::Failed => ("  X", Color::Red),
    };
    Line::from(vec![
...
fn format_relative(secs_since_epoch: u64) -> String {
    let now = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(secs_since_epoch);
    if secs_since_epoch > now {
        return "?".into();
    }
    let diff = now - secs_since_epoch;
    if diff < 60 {
        format!("{diff}s ago")
    } else if diff < 60 * 60 {
...
pub fn run<B: Backend>(term: &mut Terminal<B>, mut dash: Dashboard) -> std::io::Result<()> {
    ...
    // Kick off a tail load for every currently-Running session before the
    // first frame, so the inline execution graphs show real events on
    // first paint instead of "(no tail state yet)".
